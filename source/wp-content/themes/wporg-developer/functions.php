...
/**
 * Migrate some existing blocks over to the core code block on display.
 *
 * This is temporary hack until the blocks are migrated in the DB.
 *
 * Manual migration in the editor can be done by converting the missing block to HTML, and then to a code block.
 */
add_filter( 'the_content', function( $content ) {
	...
	// On-the-fly migrate syntaxhighlighter/code to core/code
	$content = preg_replace_callback(
		...
		function( $m ) {
			...
			return "<!-- wp:code {$m['params']} -->\n<pre class='wp-block-code'><code lang='{$lang}' class='language-{$lang} {$class}'>{$content}</code></pre>\n<!-- /wp:code -->";
		},
		$content
	);
